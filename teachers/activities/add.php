...
if ($activity_date) {
    ...
    ");
    $stmt->execute([$activity_date_only]);
...
<main class="lg:ml-64 min-h-screen flex flex-col">
    <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-poppins text-xl font-semibold text-slate-800">Add Activity</h1>
            ...
            <form method="POST" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="activity_name" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Enter activity name">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Type</label>
                        <input type="text" name="activity_type" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="e.g., Sports, Arts, Academic">
                    </div>
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Description *</label>
                    <textarea name="activity_description" required rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Enter activity description"></textarea>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Assign Students</label>
                    ...
                    <p class="text-xs text-slate-400 mt-2">Select students to assign to this activity</p>
                </div>
                
                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-inter font-medium px-6 py-3 rounded-xl transition-colors">
                        Save Activity
                    </button>
...
function renderTable(students, conflictingStudents) {
    ...
    if (!students.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-slate-400">No students found</td></tr>';
        return;
    }
