...
$stmt = $pdo->prepare("SELECT * FROM activites WHERE activity_id = ? AND activity_status = 1");
...
$stmt = $pdo->query("SELECT student_id FROM assignments WHERE activity_id = $activity_id AND assignment_status = 1");
$assigned_students = array_column($stmt->fetchAll(PDO::FETCH_ASSOC), 'student_id');
...
<main class="lg:ml-64 min-h-screen flex flex-col">
    <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-poppins text-xl font-semibold text-slate-800">Edit Activity</h1>
...
<script>
let currentPage = 1;
let selectedStudents = new Set(<?= json_encode(array_map('strval', $assigned_students)) ?>);
let searchTimeout;
let currentSearch = '';
let currentCategory = '';
