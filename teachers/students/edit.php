...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_student'])) {
        ...
        if (!empty($student_name)) {
            ...
            $stmt->execute([$student_name, $student_ic, $student_year_of_birth, $category_id, $student_parent_name, $student_parent_email, $student_parent_number, $student_notes, $student_id]);
...
<main class="lg:ml-64 min-h-screen flex flex-col">
    <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-poppins text-xl font-semibold text-slate-800">Edit Student</h1>
                <p class="text-xs text-slate-400 hidden sm:block"><?= date('l, F d, Y') ?></p>
            </div>
        </div>
        <a href="index.php?id=<?= (int)$student_id ?>" class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <form method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="student_name" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Enter student name" value="<?= htmlspecialchars($student['student_name'] ?? '') ?>">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">IC Number</label>
                        <input type="text" name="student_ic" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Enter IC (e.g. 060802030010)" maxlength="12" value="<?= htmlspecialchars($student['student_ic'] ?? '') ?>">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="student_year_of_birth" max="<?= date('Y') ?>" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="e.g. 2018" value="<?= htmlspecialchars($student['student_year_of_birth'] ?? '') ?>">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="category_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
                                ...
                                <option value="<?= $cat['category_id'] ?>" <?= ($student['category_id'] ?? '') == $cat['category_id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['category_name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="student_parent_name" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Enter parent name" value="<?= htmlspecialchars($student['student_parent_name'] ?? '') ?>">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="student_notes" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20" placeholder="Additional notes..."><?= htmlspecialchars($student['student_notes'] ?? '') ?></textarea>
                    </div>
